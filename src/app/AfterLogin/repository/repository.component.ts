...
export class RepositoryComponent implements OnInit {
  ...
  dataSource= new MatTableDataSource();
   ...
   catg_id:any;
   catg_name:any;
  constructor(private datePipe:DatePipe,private emergencyservice:VirtualEmergencyService,private spinner:NgxSpinnerService,private toastr:ToastrManager,private activatedRoute:ActivatedRoute) { }
  ...
  //For getting files
  this.emergencyservice.global_service('0','/get_repository','catg_id='+this.catg_id).subscribe(data=>{
    this.spinner.show();
     console.log(data)
    ...
    this.get_files.length=0;
      ...
      for(let i=0;i<this.Get_Uploaded_Files.length;i++){
          this.get_files.push({
            ...
            file_ext:this.Get_Uploaded_Files[i].form_path.split('.').pop().trim(),
            ...
            created_by:this.Get_Uploaded_Files[i].created_by,
             ...
             this.Get_Uploaded_Files[i].file_ext=this.Get_Uploaded_Files[i].form_path.split("/")[1].split('.').pop().trim();
             this.Get_Uploaded_Files[i].path=this.Get_Uploaded_Files[i].form_path;
   ...
   sorting(sort:any){
    ...
    this.emergencyservice.global_service('0','/get_repository','catg_id='+this.catg_id+'&flag='+flag).subscribe(data=>{
    // console.log(data)
    this.get_files.length=0;
      this.get_files=data;
      ...
      for(let i=0;i<this.get_files.length;i++){
            ...
            this.get_files[i].file_ext=this.get_files[i].file_name.split('.').pop().trim(),
  ...
  selectEvent(item:any) {
    ...
    fileChangeEvent(event:any){
       if(event.target.files[0].name.split(".")[1]=='pdf' || event.target.files[0].name.split(".")[1]=='doc' || event.target.files[0].name.split(".")[1]=='docx'){
    ...
    this.spinner.show();
      ...
      this.emergencyservice.global_service('1','/get_repository',formdata).subscribe(data=>{
        // console.log(data);
        this.check_response=data;
        if(this.check_response.suc==1){
          var dt=global_url_test.getrepository(localStorage.getItem('Email'),this.File.name,this.datePipe.transform(new Date(),'dd/MM/YYYY hh:mma'));
          this.emergencyservice.global_service('1','/post_notification',dt).subscribe(data=>{console.log(data);})
          this.toastr.successToastr(this.check_response.msg,'',{position:'top-center',animate:'slideFromTop',toastTimeout:50000});
          $("#activeCard").fadeToggle("slow");
          this.LogForm.reset();
          this.File='';
            //For getting files
         this.emergencyservice.global_service('0','/get_repository',null).subscribe(data=>{
        this.Get_Uploaded_Files.length=0;
        this.get_files.length=0;
          this.Get_Uploaded_Files=data;
          this.Get_Uploaded_Files=this.Get_Uploaded_Files.msg;
      ...
      console.log(this.Get_Uploaded_Files);
        ...
        for(let i=0;i<this.Get_Uploaded_Files.length;i++){
            this.get_files.push({
              ...
              catg_name:this.Get_Uploaded_Files[i].catg_name,
              file_name:this.Get_Uploaded_Files[i].file_name,
              file_ext:this.Get_Uploaded_Files[i].file_name.split('.').pop().trim(),
              date:this.Get_Uploaded_Files[i].created_at,
              created_by:this.Get_Uploaded_Files[i].created_by
               })
           }
